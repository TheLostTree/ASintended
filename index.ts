...
import { AvatarData, AvatarSkillDepotData, GadgetData, ProudSkillData } from './excels';
    ...
    invokeAction(action: any, abilityOrModifier: any, rootAbilityName:string){
        ...
        switch (action.$type){
            case "TriggerBullet":
                ...
                //todo: technically the ability isnt always triggered; tehres a condition but im just going to assume its goig to be triggerd immediately;
            ...
            case "TriggerAttackEvent":
                ...
                let attackFormula = action.attackEvent.attackInfo.attackProperty.damageExtra;
                
                this.indent(()=>{this.log(`Attack Event w/ Attack formula: ${attackFormula}`)})
                this.avatar.resolveEquation(attackFormula, rootAbilityName, abilityOrModifier.params);
...
class SkillDepot{
    ...
    init(avatar:Avatar){
        ...
        AvatarSkillDepotData.getSkills(this.depotId).forEach(skillId => {
            if(!skillId){
                return;
            };
        ...
        let proudSkillsGroups = AvatarSkillDepotData.getInherentProudSkillIds(this.depotId)
                                    .filter(x=>x.needAvatarPromoteLevel<=avatar.ascension)
                                    .filter(x=>x.proudSkillGroupId!=0).map(x=>x.proudSkillGroupId);

        this.proudSkillIds = proudSkillsGroups.flatMap(x=>ProudSkillData.getWithGroupId(x.toString())).map(x=>x.proudSkillId); //todo:addProps

        let proudSkillInfos = this.proudSkillIds.map(x=>ProudSkillData.get(x.toString()));

        
        for(let proudSkill of proudSkillInfos){
            ...
            let mixins = avatar.configTalent.get(openConfig);

            for(let mixin of mixins){
                avatar.mixin(mixin, params)
            }
...
class Avatar{
    ...
    resolvePercentReference(eq:string){
    ...
    resolveEquation(eq: string, ability:string, params: string[]){
        ...
        for(let key in FightProperties){
            ...
            result = result.replace(`%${key}`, (this.fightProps[key] || 0).toString());
        }
...
// console.log(ProudSkillData.data);
